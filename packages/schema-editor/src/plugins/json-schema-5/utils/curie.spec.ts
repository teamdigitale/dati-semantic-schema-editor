...
describe('uri2shortUri', () => {
  ...
  it('should return a shortened URI with domain if no parent path is found', async () => {
    const uri = 'http://eurovoc.europa.eu/100163/';
    const curie = uri2shortUri(uri);
    expect(curie).toBe('eurovoc.europa.eu:100163');
  });
});
