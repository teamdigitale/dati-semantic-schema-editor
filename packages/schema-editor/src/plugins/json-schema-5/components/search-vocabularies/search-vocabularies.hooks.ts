...
import {
  ...
  Linkset,
  PaginatedResponse,
  VocabularyCatalogQueryParams,
  VocabularyItemsQueryParams,
} from './vocabularies.models';
...
export function useSearchVocabularyItems(
  url: string,
  params?: VocabularyItemsQueryParams,
  options?: Options,
): AsyncState<PaginatedResponse> {
  const { vocabulariesApiUrl } = useConfiguration();
  const queryParams = new URLSearchParams();
  Object.entries({ limit: 10, ...params })
    .filter(([, value]) => !!value || value === 0)
    .forEach(([key, value]) => queryParams.append(key, String(value)));
  const queryString = queryParams.toString();

  const [state, setState] = useState<AsyncState<PaginatedResponse>>({ status: 'idle' });
